...
export default defineEventHandler(async (event) => {
  ...
  const response = await storageService.upload({ key: uniqueKey, data: image });

  await imageActions.createImage({ userId: user.id, key: uniqueKey, publicUrl: response.pathname });
